...
const ApplicationProcess: React.FC = () => {
  ...
  return (
    <section id="process" className="py-16 sm:py-20 md:py-24 bg-gray-50 w-full overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 w-full">
        <div className="text-center mb-12 sm:mb-16 md:mb-20">
          <div className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-red-600/10 text-red-600 font-semibold mb-4 sm:mb-6">
            <Clock className="h-3 w-3 sm:h-4 sm:w-4 mr-2" />
            <span className="text-sm sm:text-base">Quick & Easy Process</span>
          </div>
          ...
          <p className="text-base sm:text-lg md:text-xl text-gray-600 max-w-3xl sm:max-w-4xl mx-auto leading-relaxed px-2">
        ...
        {/* Process Steps */}
        <div className="relative mb-12 sm:mb-16 md:mb-20 w-full">
          {/* Connection Line */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 w-full">
            {steps.map((step, index) => (
              <div key={index} className="relative group w-full">
                {/* Step Number */}
                <div className="flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-red-600 to-red-700 text-white rounded-full font-bold text-xl sm:text-2xl mx-auto mb-4 sm:mb-6 relative z-10 shadow-lg group-hover:scale-110 transition-transform duration-300">
                  {index + 1}
                </div>
                
                {/* Step Content */}
                <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg hover:shadow-xl p-4 sm:p-6 md:p-8 text-center transition-all duration-300 transform group-hover:scale-105 border border-gray-100 group-hover:border-red-600/30 w-full">
                  <div className="text-red-600 mb-3 sm:mb-4 flex justify-center group-hover:scale-110 transition-transform duration-300">
                    {React.cloneElement(step.icon as React.ReactElement, { className: "h-6 w-6 sm:h-8 sm:w-8" })}
                  </div>
                  <h3 className="text-base sm:text-lg md:text-xl font-bold text-gray-800 mb-2 sm:mb-3">{step.title}</h3>
                  <p className="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4 leading-relaxed">{step.description}</p>
                  <div className="space-y-2">
                    <div className="bg-white border-2 border-red-600 text-red-600 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-bold">
                      {step.requirement}
                    </div>
                    <div className="text-[10px] sm:text-xs text-gray-500 flex items-center justify-center">
