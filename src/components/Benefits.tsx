...
const Benefits: React.FC = () => {
  ...
  return (
    <section id="benefits" className="py-16 sm:py-20 md:py-24 bg-white w-full overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 w-full">

        {/* Enhanced Payment System Section */}
        <div className="bg-gradient-to-r from-red-600 via-red-700 to-red-800 rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 md:p-12 text-white overflow-hidden relative mb-12 sm:mb-16 md:mb-20 w-full">
          <div className="absolute top-0 right-0 w-64 h-64 sm:w-96 sm:h-96 bg-white/10 rounded-full -translate-y-24 sm:-translate-y-48 translate-x-24 sm:translate-x-48"></div>
          <div className="absolute bottom-0 left-0 w-48 h-48 sm:w-64 sm:h-64 bg-white/5 rounded-full translate-y-16 sm:translate-y-32 -translate-x-16 sm:-translate-x-32"></div>
          
          <div className="relative z-10 w-full">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 items-center">
              <div className="w-full">
                <div className="flex items-center mb-3 sm:mb-4">
                  <Banknote className="h-6 w-6 sm:h-8 sm:w-8 text-white mr-2 sm:mr-3" />
                  <span className="text-white font-semibold text-sm sm:text-base">Payment Guarantee</span>
                </div>
                <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 sm:mb-6">Guaranteed Monthly Payments</h3>
                <p className="text-red-100 mb-6 sm:mb-8 text-sm sm:text-base md:text-lg leading-relaxed">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-6 mb-4 sm:mb-6">
                  <div className="bg-white text-red-600 px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl font-bold text-sm sm:text-lg">
                    Payment Date: 5th
                  </div>
                  <div className="flex items-center text-red-100 text-sm sm:text-base">
              ...
              <div className="w-full">
                <h4 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center">Supported Banks:</h4>
                <div className="grid grid-cols-2 gap-3 sm:gap-4 w-full">
                  {banks.map((bank, index) => (
                    <div key={index} className={`${bank.color} p-3 sm:p-4 rounded-lg sm:rounded-xl text-center font-bold shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105`}>
                      <img src={bank.logoUrl} alt={bank.name} className="h-6 sm:h-8 w-auto mx-auto mb-1 sm:mb-2" />
                      <div className="text-white text-xs sm:text-sm">{bank.name}</div>
                    </div>
                  ))}
                </div>
                <div className="mt-4 sm:mt-6 text-center">
                  <div className="inline-flex items-center px-3 sm:px-4 py-1.5 sm:py-2 bg-white/20 backdrop-blur-sm rounded-full">
                    <span className="text-xs sm:text-sm">+ Mobile Money Options</span>
                  </div>
                </div>
              </div>
            </div>
