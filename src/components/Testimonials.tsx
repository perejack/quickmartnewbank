...
const Testimonials: React.FC = () => {
  ...
  return (
    <section id="testimonials" className="py-16 sm:py-20 md:py-24 bg-white w-full overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 w-full">
        <div className="text-center mb-12 sm:mb-16 md:mb-20">
          <div className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-red-600/10 text-red-600 font-semibold mb-4 sm:mb-6">
            ...
            <span className="text-sm sm:text-base">Employee Stories</span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mb-12 sm:mb-16 w-full">
          {testimonials.map((testimonial, index) => (
            <div key={index} className="group bg-gradient-to-br from-gray-50 to-white rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 hover:shadow-2xl transition-all duration-500 transform hover:scale-105 border border-gray-100 hover:border-red-600/30 w-full">
              <div className="flex items-center mb-4 sm:mb-6">
                <Quote className="h-5 w-5 sm:h-6 sm:h-6 sm:h-8 sm:w-8 text-green-500 mr-2 sm:mr-4" />
                <div className="flex text-green-500">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star key={i} className="h-3 w-3 sm:h-4 sm:h-4 sm:h-5 sm:w-5 fill-current" />
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-center mb-3 sm:mb-4">
                <img 
                  ...
                  className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full object-cover mr-2 sm:mr-4 border-2 border-red-600"
                />
                <div>
                  <h4 className="font-bold text-gray-800 text-sm sm:text-base md:text-lg">{testimonial.name}</h4>
